...
//
//  This is BGMDriver's copy. BGMApp has its own copy with the same interface. BGMApp toggles its own
//  flag directly and toggles BGMDriver's by setting kAudioDeviceCustomPropertyDebugLoggingEnabled,
//  which BGMDriver handles by calling BGMSetDebugLoggingEnabled.
//
...
//  well tested. (In BGMDriver, logging while enabled is definitely not real-time safe -- it calls
//  syslog on the IO thread -- but it's real-time safe and cheap while disabled.)
//
